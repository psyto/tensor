...
intentRoutes.post("/validate", (req, res) => {
  try {
    ...
    for (let i = 0; i < intent.legs.length; i++) {
      ...
      if (!leg.asset) errors.push(`Leg ${i}: asset is required`);
      if (!leg.side) errors.push(`Leg ${i}: side is required`);
      if (!leg.size || leg.size <= 0) errors.push(`Leg ${i}: size must be positive`);
...
intentRoutes.post("/solve", (req, res) => {
  try {
    ...
    const result: SolverResult = {
      ...
      estimated_gas: steps.length * 200_000,
